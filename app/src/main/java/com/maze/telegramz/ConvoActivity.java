...
public class ConvoActivity extends AppCompatActivity {
    ...
    private void loadImages(TdApi.Message m) {
        ...
        client.send(new TdApi.DownloadFile(mph.photo.sizes[mph.photo.sizes.length - 1].photo.id, 1, 0, 0, false), new PhotoDownloadHandler(m));
    }

    public class PhotoDownloadHandler implements Client.ResultHandler {
        TdApi.Message m;
        TdApi.MessagePhoto mph;

        public PhotoDownloadHandler(TdApi.Message m){
            this.m = m;
            mph = (TdApi.MessagePhoto) m.content;
        }
        ...
        public void onResult(TdApi.Object object) {
            if (object.getConstructor() == TdApi.File.CONSTRUCTOR) {
                ...
                if (!f.local.isDownloadingCompleted) {
                    m.setDownloadProgress(f.local.downloadedSize / f.expectedSize);
                    client.send(new TdApi.DownloadFile(mph.photo.sizes[mph.photo.sizes.length - 1].photo.id, 1, 0, 0, false), new PhotoDownloadHandler(m));
                } else {
